local SGTZGuide4 = class("SGTZGuide4", function()
	local node =  cc.uiloader:load("ccbResources/SGTZRes/ui/guide/SGTZGuide_4.csb")
	return node
end)

function SGTZGuide4:ctor()
...
function SGTZGuide4:setFinishCallback(callback)
	self.mCallback = callback
end


function SGTZGuide4:setWorldPos(worldPoint)
	self:initShader(worldPoint)
	local pos = self:convertToNodeSpace(worldPoint)
	self.Image_shou:setPosition(pos.x,pos.y)
end

function SGTZGuide4:initShader(pos)
    ...
    local mask = display.newScale9Sprite("ccbResources/SGTZRes/image/guide/JC_zhezhao.png", pos.x, pos.y, cc.size(346, 288), capInsets)
    self.mCanva:begin()  
    mask:visit()  
