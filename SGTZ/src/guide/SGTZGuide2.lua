local SGTZGuide2 = class("SGTZGuide2", function()
	local node =  cc.uiloader:load("ccbResources/SGTZRes/ui/guide/SGTZGuide_2.csb")
	return node
end)

function SGTZGuide2:ctor()
...
function SGTZGuide2:setFinishCallback(callback)
	self.mCallback = callback
end

function SGTZGuide2:setWorldPos(worldPoint)
	self:initShader(worldPoint)
	local pos = self:convertToNodeSpace(worldPoint)
	self.Image_shou:setPosition(pos.x,pos.y)
end

function SGTZGuide2:initShader(pos)
    ...
    local mask = display.newScale9Sprite("ccbResources/SGTZRes/image/guide/JC_zhezhao.png", pos.x, pos.y, cc.size(120, 120), capInsets)
    self.mCanva:begin()  
    mask:visit()  
